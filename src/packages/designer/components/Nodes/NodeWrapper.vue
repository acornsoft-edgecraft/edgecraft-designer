...
import { DraggableEventListener, DraggableCore } from "~/packages/draggable";
import { useVueFlow } from "~/packages/designer/composables";
import { GraphNode, NodeComponent, SnapGrid } from "~/packages/designer/types";
import { NodeId, Slots } from "~/packages/designer/context";
import { getXYZPos } from "~/packages/designer/utils";
...
const onMouseEnterHandler = () => node.value.dragging && ((event: MouseEvent) => store.hooks.nodeMouseEnter.trigger({ event, node: node.value }));
const onMouseMoveHandler = () => node.value.dragging && ((event: MouseEvent) => store.hooks.nodeMouseMove.trigger({ event, node: node.value }));
const onMouseLeaveHandler = () => node.value.dragging && ((event: MouseEvent) => store.hooks.nodeMouseLeave.trigger({ event, node: node.value }));
...
onMounted(() => {
  ...
  watch([() => node.value.type, () => node.value.sourcePosition, () => node.value.targetPosition], () => nextTick(() => store.updateNodeDimensions([{ id: node.value.id, nodeElement: nodeElement.value }])));
...
<template>
  <DraggableCore :cancel="`.${store.noDragClassName}`" :handle="node.dragHandle" :disabled="!props.draggable" :scale="scaleDebounced" :grid="props.snapGrid" :enable-user-select-hack="false" @start="onDragStart" @move="onDrag" @stop="onDragStop">
    <div
      ref="node-element"
      :key="`node-${node.id}`"
      :class="[
